...
def create_brain_mask(image_data):
    """
    Create a brain mask by identifying all non-zero voxels.
    Args:
        image_data (numpy.ndarray): 3D image array.
    Returns:
...
def clip_intensity(image_data, lower_percentile=2.5, upper_percentile=97.5):
    ...
    Returns:
        numpy.ndarray: Clipped image array.
    """
...
def crop_to_bounding_box(image_data, mask_data, padding=20):
    ...
    Args:
        image_data (numpy.ndarray): 3D image array.
        mask_data (numpy.ndarray): Binary mask of the region of interest.
...
def preprocess_subject(image_paths, mask_path, padding=20):
    ...
    Args:
        image_paths (list): List of paths to modality images (e.g., ['t1c.nii', 't2f.nii']).
        mask_path (str): Path to the segmentation mask.
        padding (int): Padding around the bounding box during cropping.
    Returns:
        Tuple[numpy.ndarray, numpy.ndarray]: Preprocessed stacked modalities and mask.
    """
    images = []
    brain_mask = None
