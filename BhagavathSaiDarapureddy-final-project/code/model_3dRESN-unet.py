...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, save_directory, patience=5,
    ...
    scaler = GradScaler()
    # Initialize a list to store history for CSV export
    history = {
        'epoch': [],
        'train_loss': [],
        'train_dice': [],
...
# device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
#
# img_dir = '/home/ubuntu/DL-PROJECT/combined_data/train_images'
# mask_dir = '/home/ubuntu/DL-PROJECT/combined_data/masks'
#
...
#
#
# sav_dir='/home/ubuntu/dl_code/models'
...
#
# # train_model(model, data_loader, val_loader, criterion, optimizer, num_epochs=20, save_directory=sav_dir,num_classes=5)
# history = train_model(model, data_loader, val_loader, criterion, optimizer, 20, sav_dir, 5, 5)
#
#
# history_df=pd.DataFrame(history)
#
# history_df.to_csv('metrics.csv', index=False)
#
...
if __name__ == "__main__":
    ...
    # This is the training loop, which you only want to run when executing model.py directly
    history = train_model(model, data_loader, val_loader, criterion, optimizer, 20, sav_dir, 5, 5)
