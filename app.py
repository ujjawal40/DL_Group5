...
def hausdorff_distance(preds, true, num_classes):
    # Assuming preds and true are PyTorch tensors representing 3D volumes
    ...
    for c in range(1, num_classes):  # Assuming 0 is background
        ...
        # Validate and calculate Hausdorff distance
        if u.size == 0 or v.size == 0:
            hd = float('inf')  # Return infinity if one of the masks is completely empty
        else:
            hd1 = directed_hausdorff(u, v)[0]
            hd2 = directed_hausdorff(v, u)[0]
            hd = max(hd1, hd2)

        class_wise_hausdorff.append(hd)
...
def predicting_tab_residual():
    path=config.val_images_dir
    mask_path=config.val_mask_dir
...
def predicting_tab_unet():
    ...
    if image_selection_unet:
        ...
        images_original=torch.tensor(image_stack_original, dtype=torch.float32)
        masks_original= torch.tensor(original_seg_array, dtype=torch.long)
        fig,dice_score=unet_model(images_original, masks_original)
        st.pyplot(fig)
        st.subheader("Results")
        for i,score in enumerate(dice_score):
            st.write(f"*Dice Coefficient* class {i+1}: {score:.4f}")
...
def modelling():
    ...
    st.markdown("""
       * **Precision in Segmentation**: Offers detailed segmentation capabilities vital for accurately delineating brain tumor regions.
       * **Efficiency with 3D Data**: Specifically designed to handle 3D data, making it ideal for MRI images which are inherently three-dimensional.
       * **Adaptability**: Proven effectiveness in learning from volumetric data with limited annotations, a common challenge in medical imaging.
       * **Enhanced Feature Extraction**: Utilizes multiple levels of feature extraction to capture both high and low-level details necessary for identifying various tumor types.
       """)
...
def metric_tab():
    # Load the metrics CSV files from the config paths
    resunet_metrics_path = config.metrics_resunet  # Path to Residual UNet metrics.csv
    unet_metrics_path = config.unet_metrics  # Path to UNet metrics.csv
