...
def unet_model(images, segmentations):
    """Run prediction using a 3D U-Net model."""
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    model = UNet3D(n_channels=4, n_classes=5).to(device)
    model_path = configuration.unet_model_path
    model.load_state_dict(torch.load(model_path, map_location=device))
    images = images.unsqueeze(0) if images.dim() == 4 else images
    images, segmentations = images.to(device), segmentations.to(device)
    outputs = model(images)
    predictions = torch.argmax(outputs, dim=1)
    dice_score = dice_coefficient(outputs, segmentations, num_classes=5)
    fig = visualize_predictions(images.cpu().numpy(), segmentations.cpu().numpy(), predictions.cpu().numpy())
...
def predicting_tab_unet_with_transformation():
    st.title("3D U-Net Prediction with Image Transformation")
    path = configuration.val_images_dir
    mask_path = configuration.val_mask_dir
    images_unet = os.listdir(path)

    image_selection_unet = st.selectbox("Select an NII file for prediction with 3D UNET after Transformation", options=images_unet, key="unet_trans")

    # Select the type of transformation
    transform_options = ["None", "Flip", "Rotation", "Scale"]
    selected_transformation = st.selectbox("Select the type of transformation to apply", options=transform_options, key="transform_selection")

    if st.button("Apply Transformation and Predict"):
        ...
        fig, dice_score = unet_model(images_tensor, masks_tensor)
        st.pyplot(fig)
