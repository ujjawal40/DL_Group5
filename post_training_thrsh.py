...
def dynamic_thresholding(logits, thresholds=None):
    """
    Apply dynamic thresholds to segmentation logits.

    Args:
        logits (torch.Tensor): Raw model outputs (logits) [Batch Size, Classes, Depth, Height, Width].
        thresholds (list or None): List of thresholds for each class. If None, calculate using Otsu's method.

    Returns:
        torch.Tensor: Thresholded binary masks [Batch Size, Classes, Depth, Height, Width].
    """
    probs = F.softmax(logits, dim=1)  # Convert logits to probabilities
    thresholds = thresholds or []

    masks = torch.zeros_like(probs, dtype=torch.float32)

    for c in range(probs.shape[1]):  # Iterate over classes
        if c == 0:  # Skip background class
            continue

        if thresholds and len(thresholds) > c:
            thresh = thresholds[c]
        else:
            # Dynamic thresholding using Otsu's method or mean
            probs_np = probs[:, c, :, :, :].cpu().numpy()
            flat_probs = probs_np.flatten()
            hist, bin_edges = np.histogram(flat_probs, bins=256, range=(0, 1))
            thresh = bin_edges[np.argmax(np.cumsum(hist) > (np.sum(hist) / 2))]  # Otsu-like approach
            thresholds.append(thresh)

        masks[:, c, :, :, :] = (probs[:, c, :, :, :] >= thresh).float()
...
def evaluate_model_with_thresholding(model, data_loader, criterion, num_classes, device='cuda', thresholds=None):
    model.eval()
    total_loss = 0
    total_dice_scores = [0] * (num_classes - 1)  # Exclude background (class 0)
    updated_thresholds = [0.5] * num_classes
    total_dice_scores_pre = [0] * (num_classes - 1)
    ...
    with torch.no_grad():
        for images, segmentations in data_loader:
            ...
            thresholded_outputs, updated_thresholds = dynamic_thresholding(outputs, updated_thresholds)

            # Convert thresholded masks to predicted classes
            preds = torch.argmax(thresholded_outputs, dim=1)

            # Compute Dice scores
            dice_scores = dice_coefficient(preds, segmentations, num_classes)
            dice_scores_pre_thresh = dice_coefficient(outputs, segmentations, num_classes)
            total_dice_scores = [total_dice_scores[i] + dice_scores[i] for i in range(len(dice_scores))]
            total_dice_scores_pre = [total_dice_scores_pre[i] + dice_scores_pre_thresh[i] for i in range(len(dice_scores_pre_thresh))]

    # Normalize Dice scores across all batches
    total_dice_scores = [score / len(data_loader) for score in total_dice_scores]
    total_dice_scores_pre = [score / len(data_loader) for score in total_dice_scores_pre]

    print(f"Validation Loss: {total_loss / len(data_loader):.4f}")
    print(f"Validation Dice Scores (Class 1-4): {total_dice_scores}")
    print(f"Validation Dice Scores before thresh (Class 1-4): {total_dice_scores_pre}")
    print(f"Updated Thresholds: {updated_thresholds}")
    return total_loss / len(data_loader), total_dice_scores, updated_thresholds
...
if __name__ == "__main__":
    ...
    # Evaluate the model
    val_loss, val_dice_scores, thresholds = evaluate_model_with_thresholding(
    ...
    with torch.no_grad():
        for images, segmentations in val_loader:
            ...
            # Apply dynamic thresholding
            thresholded_outputs, _ = dynamic_thresholding(outputs, thresholds)

            # Convert thresholded masks to predicted classes
            predictions = torch.argmax(thresholded_outputs, dim=1)
